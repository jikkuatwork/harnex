...
class GenericAdapterTest < Minitest::Test
  def test_generic_adapter_uses_cli_name_as_command
    adapter = Harnex::Adapters::Generic.new("aider", ["--model", "x"])
    assert_equal ["aider"], adapter.base_command
    assert_equal ["aider", "--model", "x"], adapter.build_command
  end

  def test_build_returns_generic_for_unknown_cli
    adapter = Harnex::Adapters.build("aider", ["--model", "x"])
    assert_instance_of Harnex::Adapters::Generic, adapter
    assert_equal ["aider", "--model", "x"], adapter.build_command
  end

  def test_detects_simple_prompt_line
    adapter = Harnex::Adapters::Generic.new("aider")
